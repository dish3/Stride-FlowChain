...
function buildResult(input: RouteInput): RouteResult {
  ...
  const risk = getRisk(input.urgency, null, 0, 0);
  return {
    source: input.source,
...
export const handlePlanRoute = (req: Request, res: Response) => {
  ...
  const result = buildResult({ ...input, weight, urgency });
  return res.json(result);
};

export const handlePlanRouteWithTransport = (req: Request, res: Response) => {
  const data = req.body as RouteInput & { overrideTransport: Transport };
  if (!CITIES[data.source] || !CITIES[data.destination]) return res.status(400).json({ error: "Unknown city" });
  if (data.source === data.destination) return res.status(400).json({ error: "Source and destination must differ" });
  const weight = Math.max(1, Math.min(50000, Math.round(Number(data.weight) || 1)));
  const urgency: Urgency = data.urgency === "High" || data.urgency === "Medium" ? data.urgency : "Low";
  ...
  const eta = calculateETA(distance, transport);
  const risk = getRisk(urgency, null, 0, 0);
  ...
  const result: RouteResult = {
    ...
    cost: Math.round(distance * COST_PER_KM[transport]),
    co2: +(distance * CO2_PER_KM[transport]).toFixed(1),
    explanation: explain(transport, urgency, weight, ocean),
    reasoning: buildReasoning(transport, urgency, weight, distance, data.source, data.destination),
    confidence: computeConfidence(urgency, risk, null, false),
    disruption: null,
    trafficLevel: 0 as Intensity,
    rainLevel: 0 as Intensity,
    warning: null,
    optimized: false,
    suggestion: `Manually selected ${transport} — AI recommended ${decideTransport(weight, urgency, distance, data.source, data.destination)}.`,
    oceanRoute: ocean,
    timeline: buildTimeline({ hasPlan: true, disruption: null, optimized: false }),
  };
  return res.json(result);
...
export const handleSimulateDisruption = (req: Request, res: Response) => {
  const data = req.body as { route: RouteResult; disruption: Disruption };
  if (!["traffic", "rain", "blockage", "storm"].includes(data.disruption)) {
    return res.status(400).json({ error: "Invalid disruption" });
  }
...
export const handleApplyWhatIf = (req: Request, res: Response) => {
  const input = req.body as { route: RouteResult; traffic: number; rain: number };
  const clamp = (n: number): Intensity => (Math.max(0, Math.min(3, Math.round(n))) as Intensity);
  const data = { route: input.route, traffic: clamp(input.traffic), rain: clamp(input.rain) };
...
export const handleOptimizeRoute = (req: Request, res: Response) => {
