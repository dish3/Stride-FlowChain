...
export const CITY_REGIONS = ["India", "Asia", "Europe", "Americas", "Middle East", "Africa"];
 ...
 */
export function isOceanRoute(a, b) {
    ...
    if ((rA === "India" && rB === "Asia") || (rA === "Asia" && rB === "India"))
        return true;
 ...
 */
export function decideTransport(weight, urgency, distance, source, destination) {
    ...
    if (weight > 5000)
        return "Train 🚆";
    if (weight > 1000 && distance > 400)
        return "Train 🚆";
    return "Truck 🚛";
}
// ─── ETA ─────────────────────────────────────────────────────────────────────
export function calculateETA(distance, transport) {
...
// ─── Risk ─────────────────────────────────────────────────────────────────────
export function getRisk(urgency, disruption, traffic = 0, rain = 0) {
    let score = urgency === "High" ? 3 : urgency === "Medium" ? 2 : 1;
    if (disruption === "blockage")
        score += 3;
    if (disruption === "storm")
        score += 4; // storms hit ships hard
    score += traffic;
...
// ─── What-if delay ────────────────────────────────────────────────────────────
export function dynamicDelay(traffic, rain, transport) {
    if (transport === "Air ✈️")
        return +(rain * 0.4).toFixed(1);
    if (transport === "Ship 🚢")
        return +(rain * 2.5).toFixed(1); // storms delay ships significantly
    if (transport === "Train 🚆")
        return +(traffic * 0.3 + rain * 0.7).toFixed(1);
    return +(traffic * 1.4 + rain * 0.9).toFixed(1); // truck most affected by ground conditions
}
export function disruptionDelay(d, transport) {
    if (d === "storm") {
        if (transport === "Ship 🚢")
            return 36; // storms can delay ships by days
        if (transport === "Air ✈️")
            return 6;
        return 2;
    }
    if (d === "traffic")
        return transport === "Truck 🚛" ? 5 : transport === "Train 🚆" ? 2 : 0;
    if (d === "rain")
        return transport === "Air ✈️" ? 3 : transport === "Ship 🚢" ? 8 : 2;
    // blockage
    if (transport === "Truck 🚛")
        return 8;
    if (transport === "Train 🚆")
...
// ─── Explanation ──────────────────────────────────────────────────────────────
export function explain(transport, urgency, weight, oceanRoute) {
    if (transport === "Air ✈️")
        return oceanRoute
...
// ─── Reasoning bullets ────────────────────────────────────────────────────────
export function buildReasoning(transport, urgency, weight, distance, source, destination) {
    const out = [];
...
// ─── Confidence ───────────────────────────────────────────────────────────────
export function computeConfidence(urgency, risk, disruption, optimized) {
    let base = 92;
    if (urgency === "Medium")
        base -= 4;
    if (urgency === "Low")
